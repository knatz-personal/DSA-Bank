...
namespace WebPortal.Helpers
{
    public static class MenuHelperExtentions
    {
        public static MvcHtmlString RenderMenu(this HtmlHelper helper, MenuViewModel model)
        {
            ...
            foreach (MenuModel item in model.RootMenu)
            {
                if (model.SubMenu.Count(p => p.ParentID == item.ID) > 0)
                {
                    ...
                    foreach (MenuModel cp in model.SubMenu.Where(p => p.ParentID == item.ID))
                    {
                        result.AppendLine("<li>");
                        result.AppendLine("<a href='/" + cp.ControllerName + '/' + cp.ActionName + "' title='" +
                ...
                {
                    result.AppendLine("<li>");
                    result.AppendLine("<a href='/" + item.ControllerName + '/' + item.ActionName + "' title='" +
                                      item.Description + "'>" + item.Name +
                                      "</a>");
                }
                result.AppendLine("</li>");
        ...
        private static string RenderSubMenu(int itemId, int itemParentId, List<MenuModel> subMenus)
        {
            ...
            foreach (MenuModel item in subMenus.Where(p => p.ParentID == itemParentId))
            {
                result.AppendLine("<a href='/" + item.ControllerName + '/' + item.ActionName + "' title='" +
                                  item.Description + "'>" + item.Name +
                                  "</a>");
